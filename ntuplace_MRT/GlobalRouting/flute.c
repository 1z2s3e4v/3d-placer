...
{
    // added by choushrng
    FILE *fpost = fopen (POSTFILE, "wb");
    if(fpost == NULL) {
        printf("Error in opening %s\n", POSTFILE);
    }
    int ret = fwrite(POST9, 1, sizeof(POST9), fpost);
    fclose(fpost);

    FILE *fpowv = fopen (POWVFILE, "wb");
    if(fpowv == NULL) {
        printf("Error in opening %s\n", POWVFILE);
    }
    ret = fwrite(POWV9, 1, sizeof(POWV9), fpowv);
    fclose(fpowv);
    // end
...
#if ROUTING==1
                    nn = 2*d-2;
                    ret = fread(line, 1, d-2, fprt); linep=line;
                    for (j=d; j<nn; j++) {
                        c = charnum[*(linep++)];
                        p->rowcol[j-d] = c;
                    }
                    ret = fread(line, 1, nn/2+1, fprt); linep=line;  // last char \n
                    for (j=0; j<nn; ) {
                        c = *(linep++);
                        p->neighbor[j++] = c/16;
                        p->neighbor[j++] = c%16;
