...
CSiteRow::CSiteRow(void) : m_bottom(0), m_height(0) {}
...
float Module::GetWidth() { return GetWidth(m_z); }
float Module::GetHeight() { return GetHeight(m_z); }
...
float Module::GetHeight(float z) {
  ...
  if (m_heights.size() < 2) {
    ...
    h = m_heights[0] + (m_heights[1] - m_heights[0]) / (1 + exp(z));
  }
  return h;
}
...
CPlaceDB::CPlaceDB() : m_pTimingAnalysis(NULL) {
  m_pLib = NULL;
  m_pLEF = NULL;
  m_pCongMap = NULL;
  m_gCellWidth = 0.0;
...
bool CPlaceDB::BlockOutCore(int i) {
  if (m_modules[i].m_cx < m_coreRgn.left ||
      m_modules[i].m_cx > m_coreRgn.right ||
...
void CPlaceDB::Init() {
  ...
  m_totalFreeSpace = (m_coreRgn.right - m_coreRgn.left) *
                     (m_coreRgn.top - m_coreRgn.bottom) * (m_front - m_back);
  ...
  for (int i = 0; i < (int)m_modules.size(); i++) {
    ...
    if (m_modules[i].m_isFixed) {
      if (m_modules[i].m_cx < m_coreRgn.left ||
          m_modules[i].m_cx > m_coreRgn.right ||
          m_modules[i].m_cy < m_coreRgn.bottom ||
          m_modules[i].m_cy > m_coreRgn.top || m_modules[i].m_z < m_back ||
          m_modules[i].m_z > m_front) {
        // m_modules[i].m_isOutCore = true;
        continue;
      } else {
        m_totalFreeSpace -=
            getOverlap(m_coreRgn.left, m_coreRgn.right, m_modules[i].m_x,
                       m_modules[i].m_x + m_modules[i].m_width) *
            getOverlap(m_coreRgn.bottom, m_coreRgn.top, m_modules[i].m_y,
                       m_modules[i].m_y + m_modules[i].m_height) *
            getOverlap(m_back, m_front, m_modules[i].m_z,
                       m_modules[i].m_z + m_modules[i].m_thickness);
      }
    } else {
      m_totalMovableModuleNumber++;
      m_totalMovableModuleArea += m_modules[i].m_area;
      m_totalMovableModuleVolumn +=
          m_modules[i].m_area * m_modules[i].m_thickness;
  ...
  for (int i = 0; i < (int)m_modules.size(); i++) {
    ...
    // if (m_modules[i].m_area > 1000 * avg_area && m_modules[i].m_isFixed ==
    // false && m_modules[i].m_isOutCore == false  ) {
    if (m_modules[i].m_area > 1000 * avg_area &&
        m_modules[i].m_isFixed == false && BlockOutCore(i) == false) {
...
double CPlaceDB::GetFixBlockArea(const double &left, const double &bottom,
  ...
  assert(right > left);
  assert(top > bottom);
  // cout << "Fixed modules= " << m_partitions[partId].fixModuleList.size() <<
  // endl;
  for (int i = 0; i < (int)m_modules.size(); i++) {
    if (m_modules[i].m_isFixed) {
      a += getOverlapArea(left, bottom, right, top, m_modules[i].m_x,
                          m_modules[i].m_y,
                          m_modules[i].m_x + m_modules[i].m_width,
                          m_modules[i].m_y + m_modules[i].m_height);
    }
...
void CPlaceDB::Folding2() {
  ...
  for(unsigned int i = 0; i < m_sites3d[0].size(); i ++)
  {
          ...
          {
                  m_sites3d[0][i].m_interval[j] =
  m_sites3d[0][i].m_interval[j-1]
                          +
  ...
  for (unsigned int i = 0; i < m_modules.size(); i++) {
    ...
    {
      m_modules[i].m_z = 1;
      SetModuleOrientation(i, (m_modules[i].m_orient + 6) % 8);
      SetModuleLocation(i, m_modules[i].m_x,
    ...
    } else if (m_modules[i].m_cy < v_center) // 3
    {
      ...
      SetModuleLocation(i, core_width - m_modules[i].m_x - m_modules[i].m_width,
                        m_modules[i].m_y);
...
void CPlaceDB::Folding4() {
  ...
  for (unsigned int i = 0; i < m_modules.size(); i++) {
    ...
    {
      m_modules[i].m_z = 0;
      SetModuleLocation(i, m_modules[i].m_x - h_1_4, m_modules[i].m_y - v_1_4);
      m_modules3d[0].push_back(i);
    } else if (m_modules[i].m_cx > h_1_4 && m_modules[i].m_cx < h_3_4) // 1
    {
      ...
      if (m_modules[i].m_cy <= v_1_4)
        SetModuleLocation(i, m_modules[i].m_x - h_1_4,
                          v_1_4 - m_modules[i].m_y - m_modules[i].m_height);
      else
        SetModuleLocation(i, m_modules[i].m_x - h_1_4,
                          core_height + v_1_4 - m_modules[i].m_y -
    ...
    } else if (m_modules[i].m_cy > v_1_4 && m_modules[i].m_cy < v_3_4) // 3
    {
      ...
      if (m_modules[i].m_cx <= h_1_4)
        SetModuleLocation(i, h_1_4 - m_modules[i].m_x - m_modules[i].m_width,
                          m_modules[i].m_y - v_1_4);
      else
        SetModuleLocation(
            i, core_width + h_1_4 - m_modules[i].m_x - m_modules[i].m_width,
            m_modules[i].m_y - v_1_4);
      m_modules3d[3].push_back(i);
...
void CPlaceDB::SetModuleOrientationCenter(const int &moduleId,
                                          const int &orient) {
...
void CPlaceDB::PrintModules() {
  ...
  for (int i = 0; i < size; i++) {
    ...
    printf("Block %-15s at lb(%5g, %5g) missalign:%d center(%5g, %5g) w= %-5g "
           "h= %-5g pin # = %d\n",
           m_modules[i].GetName().c_str(), m_modules[i].GetX(),
           m_modules[i].GetY(),
           (int)(m_modules[i].GetX()) % (int)(m_sites[0].m_step),
           m_modules[i].m_cx, m_modules[i].m_cy, m_modules[i].GetWidth(),
           m_modules[i].GetHeight(), m_modules[i].m_pinsId.size());
    for (int j = 0; j < (int)m_modules[i].m_pinsId.size(); j++) {
      ...
      printf("   Pin rel(%5g, %5g) abs(%5g, %5g)  module_size(%5g,%5g)\n",
             m_pins[pinId].xOff, m_pins[pinId].yOff, m_pins[pinId].absX,
             m_pins[pinId].absY, m_modules[i].GetWidth(),
...
int CPlaceDB::CreateModuleNameMap() {
  ...
  for (int i = 0; i < size; ++i) {
    // 2005-12-18
    if (false ==
        (m_moduleMapNameId.insert(pair<string, int>(m_modules[i].GetName(), id))
             .second)) {
    ...
    {
        cerr << "Error, block name duplicate: " << m_modules[i].GetName() <<
    endl; return -1;
    }
...
int CPlaceDB::CreateNetNameMap() {
  ...
  for (int i = 0; i < size; ++i) {
    // 2005-12-18
    if (false ==
        (m_netMapNameId.insert(pair<string, int>(m_netsName[i], id)).second)) {
...
void CPlaceDB::ClearModuleNameMap() { m_moduleMapNameId.clear(); }
...
double CPlaceDB::GetNetLength(vector<int> &pinsId, double *xwire,
                              double *ywire) {
  if (pinsId.size() <= 1) {
    if (xwire != NULL)
      *xwire = 0;
...
double CPlaceDB::CalcHPWL() {
  vector<double> minX(m_totalLayer, 0), minY(m_totalLayer, 0), maxX(m_totalLayer, 0), maxY(m_totalLayer, 0);
  double HPWL = 0;
  double cx, cy;
  int pid;
  vector<bool> firstPin(m_totalLayer,false);
  for (int i = 0; i < (int)m_nets.size(); i++) {
    vector<bool> firstPin(m_totalLayer, true);
    for (int j = 0; j < (int)m_nets[i].size(); j++) {
      pid = m_nets[i][j];
	  ...
	  if(m_modules[m_pins[pid].moduleId].m_isVia){
		for(int k=0;k<m_totalLayer;++k){
			
		}
	  }
	  else if(!m_modules[m_pins[pid].moduleId].m_isFiller){
		int layer = m_modules[m_pins[pid].moduleId].GetZ();
		if (firstPin[layer]) {
			minX[layer] = maxX[layer] = cx;
			minY[layer] = maxY[layer] = cy;
			firstPin[layer] = false;
		} else {
			minX[layer] = min(minX[layer], cx);
			maxX[layer] = max(maxX[layer], cx);
			minY[layer] = min(minY[layer], cy);
			maxY[layer] = max(maxY[layer], cy);
		}
	  }
    }
	for(int k=0;k<m_totalLayer;++k){
		assert(maxX[k] >= minX[k]);
		assert(maxY[k] >= minY[k]);

		HPWL += ((maxX[k] - minX[k]) + (maxY[k] - minY[k]));
	}
  }
...
//	m_partitions.push_back( Partition( -1, m_bboxHeight, 0, 0, m_bboxWidth,
V_CUT ) );
//
...
//	for( int i=0; i<(int)m_modules.size(); i++ )
//	{
//		fprintf( out, "%s hardrectilinear 4 (0, 0) (0, %d) (%d, %d) (%d,
//0)\n", 			m_modules[i].m_name.c_str(), 			m_modules[i].m_height,
...
//				fprintf( out, "%s B\n",
//m_modules[m_nets[i][j]].m_name.c_str() ); 			else 				fprintf( out, "%s B\n",
...
void CPlaceDB::RestoreCoreRgn(void) { m_coreRgn = m_coreRgn2; }

// void CPlaceDB::GetRegionWeightedCenter(double left, double bottom, double
// right, double top, double& x, double& y)
//{
//    //=============================================
...
CPoint CPlaceDB::GetRegionWeightedCenter(const double &left,
                                         const double &right,
                                         const double &bottom,
                                         const double &top) {
  // FIXME
  ...
  if ((top <= site_bottom) ||
      (bottom >= m_sites[m_sites.size() - 1].m_bottom + unit_height)) {
    return CPoint(0, 0);
  }

  int bottom_index =
      static_cast<int>(floor((bottom - site_bottom) / unit_height));
  ...
  for (int i = bottom_index + 1; i < top_index - 1; i++) {
    ...
    for (int j = 0; j < (signed)currentInterval.size(); j = j + 2) {
      ...
      if (interval_left >= right || interval_right <= left)
        continue;
      ...
      double effective_area =
          (effective_right - effective_left) * effective_height;
  ...
  // BUG: 2005/03/16
  double bottom_reminder =
      m_sites[bottom_index].m_bottom + unit_height - bottom;
  ...
  if (top_index - bottom_index > 1) {
    if (bottom_reminder > 0 && bottom_index >= 0) {
      ...
      double centerY =
          m_sites[i].m_bottom + unit_height - (bottom_reminder / 2.0);
      ...
      cerr << "bottom: " << bottom << " bottom_index: " << bottom_index
           << " m_sites[bottom_index].m_bottom: "
           << m_sites[bottom_index].m_bottom << endl;
      cerr << "unit_height: " << unit_height
           << " m_sites.size(): " << (int)m_sites.size() << endl;
    }
    ...
    if (top_reminder > 0 && top_index - 1 < static_cast<int>(m_sites.size())) {
      ...
      cerr << "top: " << top << " top_index: " << top_index
           << " m_sites[top_index].m_bottom: " << m_sites[top_index].m_bottom
           << endl;
    }
  ...
  //m_sites[bottom_index].m_bottom, 																								top_index, m_sites[top_index].m_bottom );
  //
  //		printf("left: %f, right: %f, bottom: %f, top: %f\n", left, right,
  //bottom, top ); 		printf("unit_height: %f, sites size: %d\n", unit_height,
  //m_sites.size() ); 		printf("sites bottom: %f, sites top: %f\n",
...
CPoint CPlaceDB::GetCoreCenter(void) {
  ...
  if (nFixedPin == 0) // no pads
    return CPoint(0.5 * (m_coreRgn.top + m_coreRgn.bottom),
                  0.5 * (m_coreRgn.left + m_coreRgn.right));
  else
    return CPoint(sumX / static_cast<double>(nFixedPin),
                  sumY / static_cast<double>(nFixedPin));
}
...
void CPlaceDB::UpdatePinPosition() {
  for (unsigned int id = 0; id < m_modules.size(); id++)
    for (unsigned int i = 0; i < m_modules[id].m_pinsId.size(); i++)
      CalcPinLocation(m_modules[id].m_pinsId[i], m_modules[id].m_cx,
                      m_modules[id].m_cy);
}
...
void CPlaceDB::ShowRows() {
  ...
  for (int i = 0; i < (int)m_sites.size(); i++) {
    printf("   ROW %d y %10.0f    x %10.0f -- %10.0f -- %10.0f \n", i,
           m_sites[i].m_bottom, m_sites[i].m_interval[0],
           m_sites[i].m_interval[1], m_sites[i].m_height);
  }
}
...
void CPlaceDB::OutputMatlabFigure(const char *filename) {
  ...
  fprintf(out, "\ntitle('%s, block= %d, net= %d, HPWL= %.0f')\n", filename,
          (int)m_modules.size(), m_nNets, GetHPWL());
  ...
  fprintf(
      out,
      "%%rectangle( 'Position', [%.2f, %.2f, %.2f, %.2f], 'LineStyle', ':' )\n",
      m_coreRgn.left, m_coreRgn.bottom, m_coreRgn.right - m_coreRgn.left,
      m_coreRgn.top - m_coreRgn.bottom);

  // fprintf( out, "rectangle( 'Position', [%d,%d,%d,%d], 'FaceColor', [0.5 0.5
  // 0.5] )\n", 					0, 0, m_bboxWidth, m_bboxHeight ); fprintf( out, "rectangle(
  ...
  for (int i = 0; i < (int)m_modules.size(); i++) {
    ...
    if ((int)m_modules.size() < 100000 || m_modules[i].m_isFixed) {
      ...
      fprintf(out, "rectangle( 'Position', [%.2f, %.2f, %.2f, %.2f] )\n",
              m_modules[i].GetX(), m_modules[i].GetY(), w, h);
      fprintf(out, "line( [%.2f, %.2f], [%.2f, %.2f], 'Color', 'Black'  )\n",
              m_modules[i].GetX() + w * 0.75, m_modules[i].GetX() + w,
              m_modules[i].GetY() + h, m_modules[i].GetY() + h * 0.5);
    ...
    if (m_modules[i].m_isFixed) {
      ...
      fprintf(out, "rectangle( 'Position', [%.2f, %.2f, %.2f, %.2f] )\n",
              m_modules[i].GetX() + border, m_modules[i].GetY() + border,
  ...
  //	fprintf( out, "text( %f, %f, '%s', 'HorizontalAlignment', 'center',
  //'HorizontalAlignment', 'center' )\n", 					m_modules[i].m_cx, 					m_modules[i].m_cy,
  ...
  //	//fprintf( out, "rectangle( 'Position', [%d,%d,%d,%d], 'FaceColor', [1 1
  //1] )\n",
  //	//				x,
  //	//				y,
  //	//				1,
  ...
  //
  // 		    fprintf( out, "rectangle( 'Position', [%.2f, %.2f, %.2f, %.2f],
  // 'LineStyle', ':' )\n", 						    m_partitions[i].left, 						    m_partitions[i].bottom,
  // 						    m_partitions[i].right-m_partitions[i].left,
  // 						    m_partitions[i].top-m_partitions[i].bottom
  // );
  //
  //
  //    //         if( m_partitions[i].childPart1 == 0 &&
  //    m_partitions[i].childPart0 == 0)
  // 			//	continue;
  //
  // 			//double x1, x2, y1, y2;
  //    //         x1 = x2 = y1 = y2 = 0;
  ...
  //    //             {
  // 			//	    x1 = x2 = m_partitions[ m_partitions[i].childPart0
  // ].right;
  //    //             }
  //    //             else if( m_partitions[i].childPart1 != 0 )
  //    //             {
  // 			//	    x1 = x2 = m_partitions[ m_partitions[i].childPart1
  // ].left;
  //    //             }
  //
  // 			//}
  ...
  //    //             {
  // 			//	    y1 = y2 =
  // m_partitions[m_partitions[i].childPart0].top;
  //    //             }
  //    //             else if( m_partitions[i].childPart1 != 0 )
  //    //             {
  // 			//	    y1 = y2 =
  // m_partitions[m_partitions[i].childPart1].bottom;
  //    //             }
  // 			//}
  // 			//
  // 			////                    x1 x2    y1 y2
...
//     CalcHPWL();
//     string
//     plt_filename,net_filename,fixed_filename,module_filename,move_filename,
//     dummy_filename, pin_filename; FILE* pPlt; FILE* pNet; FILE* pFixed; FILE*
//     pMod; FILE* pMove; FILE* pDummy;   // donnie 2006-03-02 FILE* pPin;   //
//     indark 2006-04-24 if(withZoom){ 	plt_filename = net_filename =
//     fixed_filename = module_filename =
...
//     // output title
//     fprintf( pPlt, "\nset title \" %s, block= %d, net= %d, HPWL= %.0f \" font
//     \"Times, 22\"\n\n",
...
//     if (withZoom)
//     {
// 	fprintf( pPlt, "plot[:][:] '%s' w l lt 3, '%s' w l lt 4, '%s' w l lt 1,
// '%s' w l lt 7, '%s' w l lt 5, '%s' w l lt 2\n\n", 		fixed_filename.c_str(),
// module_filename.c_str(), move_filename.c_str(), 		net_filename.c_str(),
// dummy_filename.c_str(), pin_filename.c_str() );
//     }
//     else
//     {
// 	string drawType[2];
...
// 	int type = 0;
// 	if( gArg.CheckExist( "cong" ) && (int)m_modules.size()< 50000 ) //
// filled color 	    type = 1; 	fprintf( pPlt, "plot[:][:] '-' w l lt 3, '-' w %s lt
// 4, '-' w l lt 1, '-' w l lt 7, '-' w l lt 5, '-' w l lt 2\n\n",
// 		drawType[type].c_str() );
...
// !m_modules[i].m_isMacro ) 	if( !m_modules[i].m_isFixed &&
// !m_modules[i].m_isDummy )
// 	{
// 	    if( (int)m_modules.size()< 50000 || m_modules[i].m_height >
// m_rowHeight )
// 	    {
// 		// draw blocks
// 		fprintf( pMod, "%.3f, %.3f\n", x, y );
...
// 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.8, y+h );
// 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.8 - w*0.1, y+h -
// h*0.1 ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.8, y+h ); 			    fprintf( pMod, "%.3f,
// %.3f\n\n", x+w*0.8 + w*0.1, y+h - h*0.1 ); 			    break; 			case 1:		//W
// 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.5, y+h*0.8 );
// 			    fprintf( pMod, "%.3f, %.3f\n", x, y + h*0.8 );
// 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.1, y+h*0.9 );
// 			    fprintf( pMod, "%.3f, %.3f\n", x, y + h*0.8 );
// 			    fprintf( pMod, "%.3f, %.3f\n\n", x + w*0.1, y + h*0.7
// ); 			    break; 			case 2:		//S 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.2,
// y+h*0.5 ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.2, y ); 			    fprintf( pMod, "%.3f,
// %.3f\n", x+w*0.2-w*0.1, y+h*0.1 ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.2, y
// ); 			    fprintf( pMod, "%.3f, %.3f\n\n", x+w*0.2 + w*0.1, y+h*0.1 ); 			    break; 			case
// 3:		//E 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.5, y+h*0.2 ); 			    fprintf(
// pMod, "%.3f, %.3f\n", x+w, y+h*0.2 ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w - w
// * 0.1, y+h*0.2 + h * 0.1  ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w, y+h*0.2 );
// 			    fprintf( pMod, "%.3f, %.3f\n\n", x+w - w * 0.1, y+h*0.2 -
// h * 0.1 );
...
// ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.2, y + h ); 			    fprintf( pMod, "%.3f,
// %.3f\n", x+w*0.2 + w*0.1, y + h*0.9 ); 			    fprintf( pMod, "%.3f, %.3f\n\n",
// x+w*0.2-w*0.1, y + h*0.9 ); 			    break; 			case 5:		//FW 			    fprintf( pMod,
// "%.3f, %.3f\n", x+w*0.5, y+h*0.8 ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w, y + h
// *0.8 ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w-w*0.1, y + h *0.9 ); 			    fprintf(
// pMod, "%.3f, %.3f\n", x+w, y + h *0.8 ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w
// -w*0.1, y + h *0.7 ); 			    fprintf( pMod, "%.3f, %.3f\n\n", x+w-w*0.1, y + h *0.9
// ); 			    break; 			case 6:		//FS 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.8,
// y+h*0.5 ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.8, y  ); 			    fprintf( pMod,
// "%.3f, %.3f\n", x+w*0.7, y + h * 0.1 ); 			    fprintf( pMod, "%.3f, %.3f\n",
// x+w*0.8, y ); 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.9, y + h * 0.1); 			    fprintf(
// pMod, "%.3f, %.3f\n\n", x+w*0.7, y + h * 0.1 ); 			    break; 			case 7:
// //FE 			    fprintf( pMod, "%.3f, %.3f\n", x+w*0.5, y+h*0.2 ); 			    fprintf( pMod, "%.3f,
// %.3f\n", x, y + h*0.2  ); 			    fprintf( pMod, "%.3f, %.3f\n", x + w*0.1, y + h*0.1
// ); 			    fprintf( pMod, "%.3f, %.3f\n", x, y + h*0.2  ); 			    fprintf( pMod, "%.3f,
// %.3f\n", x + w*0.1, y + h*0.3  ); 			    fprintf( pMod, "%.3f, %.3f\n\n", x + w*0.1,
// y + h*0.1  ); 			    break;
...
// 			case 0:		//N
// 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.8, y+h*0.5
// ); 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.8, y+h ); 				fprintf( pFixed, "%.3f,
// %.3f\n", x+w*0.8 - w*0.1, y+h - h*0.1 ); 				fprintf( pFixed, "%.3f, %.3f\n",
// x+w*0.8, y+h ); 				fprintf( pFixed, "%.3f, %.3f\n\n", x+w*0.8 + w*0.1, y+h -
// h*0.1 ); 				break; 			case 1:		//W 				fprintf( pFixed, "%.3f, %.3f\n",
// x+w*0.5, y+h*0.8 ); 				fprintf( pFixed, "%.3f, %.3f\n", x, y + h*0.8 ); 				fprintf(
// pFixed, "%.3f, %.3f\n", x+w*0.1, y+h*0.9 ); 				fprintf( pFixed, "%.3f, %.3f\n",
// x, y + h*0.8 ); 				fprintf( pFixed, "%.3f, %.3f\n\n", x + w*0.1, y + h*0.7 );
// 				break;
// 			case 2:		//S
// 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.2, y+h*0.5
// ); 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.2, y ); 				fprintf( pFixed, "%.3f,
// %.3f\n", x+w*0.2-w*0.1, y+h*0.1 ); 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.2,
// y ); 				fprintf( pFixed, "%.3f, %.3f\n\n", x+w*0.2 + w*0.1, y+h*0.1 ); 				break;
// 			case 3:		//E
// 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.5, y+h*0.2
// ); 				fprintf( pFixed, "%.3f, %.3f\n", x+w, y+h*0.2 ); 				fprintf( pFixed, "%.3f,
// %.3f\n", x+w - w * 0.1, y+h*0.2 + h * 0.1  ); 				fprintf( pFixed, "%.3f,
// %.3f\n", x+w, y+h*0.2 ); 				fprintf( pFixed, "%.3f, %.3f\n\n", x+w - w * 0.1,
// y+h*0.2 - h * 0.1 );

// 				break;
// 			case 4:		//FN
// 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.2, y+h*0.5
// ); 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.2, y + h ); 				fprintf( pFixed, "%.3f,
// %.3f\n", x+w*0.2-w*0.1, y + h*0.9 ); 				fprintf( pFixed, "%.3f, %.3f\n",
// x+w*0.2, y + h ); 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.2 + w*0.1, y + h*0.9
// ); 				fprintf( pFixed, "%.3f, %.3f\n\n", x+w*0.2-w*0.1, y + h*0.9 ); 				break; 			case
// 5:		//FW 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.5, y+h*0.8 );
// 				fprintf( pFixed, "%.3f, %.3f\n", x+w, y + h *0.8
// ); 				fprintf( pFixed, "%.3f, %.3f\n", x+w-w*0.1, y + h *0.9 ); 				fprintf( pFixed,
// "%.3f, %.3f\n", x+w, y + h *0.8 ); 				fprintf( pFixed, "%.3f, %.3f\n", x+w
// -w*0.1, y + h *0.7 ); 				fprintf( pFixed, "%.3f, %.3f\n\n", x+w-w*0.1, y + h
// *0.9 ); 				break; 			case 6:		//FS 				fprintf( pFixed, "%.3f, %.3f\n",
// x+w*0.8, y+h*0.5 ); 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.8, y  ); 				fprintf(
// pFixed, "%.3f, %.3f\n", x+w*0.7, y + h * 0.1 ); 				fprintf( pFixed, "%.3f,
// %.3f\n", x+w*0.8, y ); 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.9, y + h *
// 0.1); 				fprintf( pFixed, "%.3f, %.3f\n\n", x+w*0.7, y + h * 0.1 ); 				break; 			case
// 7:		//FE 				fprintf( pFixed, "%.3f, %.3f\n", x+w*0.5, y+h*0.2 );
// 				fprintf( pFixed, "%.3f, %.3f\n", x, y + h*0.2 );
// 				fprintf( pFixed, "%.3f, %.3f\n", x + w*0.1, y + h*0.1
// ); 				fprintf( pFixed, "%.3f, %.3f\n", x, y + h*0.2  ); 				fprintf( pFixed, "%.3f,
// %.3f\n", x + w*0.1, y + h*0.3  ); 				fprintf( pFixed, "%.3f, %.3f\n\n", x +
// w*0.1, y + h*0.1  ); 				break;

// 			default:
// 				fprintf( pFixed, "%.3f, %.3f\n", x, y );
// 				fprintf( pFixed, "%.3f, %.3f\n\n", x + w, y +h
// ); 				fprintf( pFixed, "%.3f, %.3f\n", x + w, y ); 				fprintf( pFixed, "%.3f,
// %.3f\n\n", x, y +h ); 				cerr << "Orientation Error " << endl;
...
//     double threshold =
//     (m_coreRgn.top-m_coreRgn.bottom+m_coreRgn.right-m_coreRgn.left) * 0.02;
...
// 	    if( m_modules[i].m_isFixed || m_modules[i].m_height > m_rowHeight *
// 2 )
// 	    {
// 		fprintf( pPin, "     # pin for block %s\n",
// m_modules[i].GetName().c_str() ); 		for (int j = 0 ; j <
// (int)m_modules[i].m_pinsId.size(); j++)
// 		{
// 		    int pin_id =  m_modules[i].m_pinsId[j];
// 		    pin_x = this->m_pins[pin_id].absX;
// 		    pin_y = this->m_pins[pin_id].absY;
// 		    fprintf( pPin, "%.3f, %.3f\n",   pin_x - pin_width*0.5, pin_y -
// pin_width*0.5 ); 		    fprintf( pPin, "%.3f, %.3f\n",   pin_x - pin_width*0.5,
// pin_y + pin_width*0.5 ); 		    fprintf( pPin, "%.3f, %.3f\n",   pin_x +
// pin_width*0.5, pin_y + pin_width*0.5 ); 		    fprintf( pPin, "%.3f, %.3f\n", pin_x
// + pin_width*0.5, pin_y - pin_width*0.5 ); 		    fprintf( pPin, "%.3f, %.3f\n\n",
...
//     if (withZoom)
//     {
// 	fprintf( pPlt, "splot[:][:][:] '%s' w l 3, '%s' w l 4, '%s' w l 1, '%s'
// w l 7, '%s' w l 5, '%s' w l 2\n\n", 		fixed_filename.c_str(),
// module_filename.c_str(), move_filename.c_str(), 		net_filename.c_str(),
// dummy_filename.c_str(), pin_filename.c_str() );
//     }
//     else
...
// 	int type = 0;
// 	if( gArg.CheckExist( "cong" ) && (int)m_modules.size()< 50000 ) //
// filled color 	    type = 1; 	fprintf( pPlt, "splot[:][:][:] '-' w l 3, '-' w %s 4,
// '-' w l 1, '-' w l 7, '-' w l 5, '-' w l 2\n\n", 		drawType[type].c_str() );
...
//     layer*layerThickness ); 	fprintf( pFixed, "%.3f, %.3f, %.3f\n",
//     m_coreRgn.right, m_coreRgn.bottom, layer*layerThickness ); 	fprintf(
//     pFixed, "%.3f, %.3f, %.3f\n", m_coreRgn.right, m_coreRgn.top,
//     layer*layerThickness ); 	fprintf( pFixed, "%.3f, %.3f, %.3f\n",
//     m_coreRgn.left, m_coreRgn.top, layer*layerThickness ); 	fprintf( pFixed,
//     "%.3f, %.3f, %.3f\n\n", m_coreRgn.left, m_coreRgn.bottom,
//     layer*layerThickness );
//     	/*fprintf( pFixed, "\n# die area\n" );
//     	fprintf( pFixed, "%.3f, %.3f, %d\n", m_dieArea.left,  m_dieArea.bottom,
//     layer ); 	fprintf( pFixed, "%.3f, %.3f, %d\n", m_dieArea.right,
//     m_dieArea.bottom, layer ); 	fprintf( pFixed, "%.3f, %.3f, %d\n",
//     m_dieArea.right, m_dieArea.top, layer ); 	fprintf( pFixed, "%.3f, %.3f,
//     %d\n", m_dieArea.left,  m_dieArea.top, layer ); 	fprintf( pFixed, "%.3f,
...
// 			case 0:		//N
// 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.8, y+h*0.5, z
// ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.8, y+h, z ); 			    fprintf( pMod,
// "%.3f, %.3f, %.3f\n", x+w*0.8 - w*0.1, y+h - h*0.1, z ); 			    fprintf( pMod,
// "%.3f, %.3f, %.3f\n", x+w*0.8, y+h, z ); 			    fprintf( pMod, "%.3f, %.3f,
// %.3f\n\n", x+w*0.8 + w*0.1, y+h - h*0.1, z ); 			    break; 			case 1:		//W
// 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.5, y+h*0.8, z
// ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x, y + h*0.8, z ); 			    fprintf( pMod,
// "%.3f, %.3f, %.3f\n", x+w*0.1, y+h*0.9, z ); 			    fprintf( pMod, "%.3f, %.3f,
// %.3f\n", x, y + h*0.8, z ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n\n", x + w*0.1,
// y + h*0.7, z ); 			    break; 			case 2:		//S 			    fprintf( pMod, "%.3f, %.3f,
// %.3f\n", x+w*0.2, y+h*0.5, z ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.2,
// y, z ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.2-w*0.1, y+h*0.1, z );
// 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.2, y, z
// ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n\n", x+w*0.2 + w*0.1, y+h*0.1, z );
// 			    break;
// 			case 3:		//E
// 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.5, y+h*0.2, z
// ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w, y+h*0.2, z ); 			    fprintf( pMod,
// "%.3f, %.3f, %.3f\n", x+w - w * 0.1, y+h*0.2 + h * 0.1, z  ); 			    fprintf( pMod,
// "%.3f, %.3f, %.3f\n", x+w, y+h*0.2, z ); 			    fprintf( pMod, "%.3f, %.3f,
// %.3f\n\n", x+w - w * 0.1, y+h*0.2 - h * 0.1, z );

// 			    break;
// 			case 4:		//FN
// 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.2, y+h*0.5, z
// ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.2, y + h, z ); 			    fprintf( pMod,
// "%.3f, %.3f, %.3f\n", x+w*0.2-w*0.1, y + h*0.9, z ); 			    fprintf( pMod, "%.3f,
// %.3f, %.3f\n", x+w*0.2, y + h, z ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n",
// x+w*0.2 + w*0.1, y + h*0.9, z ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n\n",
// x+w*0.2-w*0.1, y + h*0.9, z ); 			    break; 			case 5:		//FW 			    fprintf(
// pMod, "%.3f, %.3f, %.3f\n", x+w*0.5, y+h*0.8, z ); 			    fprintf( pMod, "%.3f,
// %.3f, %.3f\n", x+w, y + h *0.8, z ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n",
// x+w-w*0.1, y + h *0.9, z ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w, y + h
// *0.8, z ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w -w*0.1, y + h *0.7, z );
// 			    fprintf( pMod, "%.3f, %.3f, %.3f\n\n", x+w-w*0.1, y + h
// *0.9, z ); 			    break; 			case 6:		//FS 			    fprintf( pMod, "%.3f, %.3f,
// %.3f\n", x+w*0.8, y+h*0.5, z ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.8,
// y, z  ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.7, y + h * 0.1, z );
// 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.8, y, z
// ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.9, y + h * 0.1, z); 			    fprintf(
// pMod, "%.3f, %.3f, %.3f\n\n", x+w*0.7, y + h * 0.1, z ); 			    break; 			case 7:
// //FE 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x+w*0.5, y+h*0.2, z ); 			    fprintf(
// pMod, "%.3f, %.3f, %.3f\n", x, y + h*0.2, z  ); 			    fprintf( pMod, "%.3f, %.3f,
// %.3f\n", x + w*0.1, y + h*0.1, z  ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x,
// y + h*0.2, z  ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x + w*0.1, y + h*0.3, z
// ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n\n", x + w*0.1, y + h*0.1, z  ); 			    break;

// 			default:
// 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x, y, z );
// 			    fprintf( pMod, "%.3f, %.3f, %.3f\n\n", x + w, y +h, z
// ); 			    fprintf( pMod, "%.3f, %.3f, %.3f\n", x + w, y, z ); 			    fprintf( pMod, "%.3f,
// %.3f, %.3f\n\n", x, y +h, z ); 			    cerr << "Orientation Error " << endl;
...
// 			case 0:		//N
// 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w*0.8, y+h*0.5,
// z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w*0.8, y+h, z ); 				fprintf(
// pFixed, "%.3f, %.3f, %.3f\n", x+w*0.8 - w*0.1, y+h - h*0.1, z ); 				fprintf(
// pFixed, "%.3f, %.3f, %.3f\n", x+w*0.8, y+h, z ); 				fprintf( pFixed, "%.3f,
// %.3f, %.3f\n\n", x+w*0.8 + w*0.1, y+h - h*0.1, z ); 				break; 			case 1:
// //W 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w*0.5, y+h*0.8, z ); 				fprintf(
// pFixed, "%.3f, %.3f, %.3f\n", x, y + h*0.8, z ); 				fprintf( pFixed, "%.3f,
// %.3f, %.3f\n", x+w*0.1, y+h*0.9, z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n",
// x, y + h*0.8, z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n\n", x + w*0.1, y +
// h*0.7, z ); 				break; 			case 2:		//S 				fprintf( pFixed, "%.3f, %.3f,
// %.3f\n", x+w*0.2, y+h*0.5, z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n",
// x+w*0.2, y, z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w*0.2-w*0.1,
// y+h*0.1, z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w*0.2, y, z );
// 				fprintf( pFixed, "%.3f, %.3f, %.3f\n\n", x+w*0.2 + w*0.1,
// y+h*0.1, z ); 				break; 			case 3:		//E 				fprintf( pFixed, "%.3f, %.3f,
// %.3f\n", x+w*0.5, y+h*0.2, z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w,
// y+h*0.2, z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w - w * 0.1, y+h*0.2 +
// h * 0.1, z  ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w, y+h*0.2, z );
// 				fprintf( pFixed, "%.3f, %.3f, %.3f\n\n", x+w - w * 0.1,
// y+h*0.2 - h * 0.1, z );
...
// 			case 4:		//FN
// 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w*0.2, y+h*0.5,
// z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w*0.2, y + h, z ); 				fprintf(
// pFixed, "%.3f, %.3f, %.3f\n", x+w*0.2-w*0.1, y + h*0.9, z ); 				fprintf( pFixed,
// "%.3f, %.3f, %.3f\n", x+w*0.2, y + h, z ); 				fprintf( pFixed, "%.3f, %.3f,
// %.3f\n", x+w*0.2 + w*0.1, y + h*0.9, z ); 				fprintf( pFixed, "%.3f, %.3f,
// %.3f\n\n", x+w*0.2-w*0.1, y + h*0.9, z ); 				break; 			case 5:		//FW
// 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w*0.5, y+h*0.8,
// z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w, y + h *0.8, z ); 				fprintf(
// pFixed, "%.3f, %.3f, %.3f\n", x+w-w*0.1, y + h *0.9, z ); 				fprintf( pFixed,
// "%.3f, %.3f, %.3f\n", x+w, y + h *0.8, z ); 				fprintf( pFixed, "%.3f, %.3f,
// %.3f\n", x+w -w*0.1, y + h *0.7, z ); 				fprintf( pFixed, "%.3f, %.3f,
// %.3f\n\n", x+w-w*0.1, y + h *0.9, z ); 				break; 			case 6:		//FS
// 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w*0.8, y+h*0.5,
// z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x+w*0.8, y, z  ); 				fprintf(
// pFixed, "%.3f, %.3f, %.3f\n", x+w*0.7, y + h * 0.1, z ); 				fprintf( pFixed,
// "%.3f, %.3f, %.3f\n", x+w*0.8, y, z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n",
// x+w*0.9, y + h * 0.1, z); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n\n", x+w*0.7, y
// + h * 0.1, z ); 				break; 			case 7:		//FE 				fprintf( pFixed, "%.3f,
// %.3f, %.3f\n", x+w*0.5, y+h*0.2, z ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n",
// x, y + h*0.2, z  ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x + w*0.1, y +
// h*0.1, z  ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x, y + h*0.2, z  );
// 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x + w*0.1, y +
// h*0.3, z  ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n\n", x + w*0.1, y + h*0.1, z
// ); 				break;

// 			default:
// 				fprintf( pFixed, "%.3f, %.3f, %.3f\n", x, y, z
// ); 				fprintf( pFixed, "%.3f, %.3f, %.3f\n\n", x + w, y +h, z ); 				fprintf(
// pFixed, "%.3f, %.3f, %.3f\n", x + w, y, z ); 				fprintf( pFixed, "%.3f, %.3f,
// %.3f\n\n", x, y +h, z ); 				cerr << "Orientation Error " << endl;
...
// pin_y - pin_width*0.5, z ); 		    fprintf( pPin, "%.3f, %.3f, %.3f\n",   pin_x -
// pin_width*0.5, pin_y + pin_width*0.5, z ); 		    fprintf( pPin, "%.3f, %.3f,
// %.3f\n",   pin_x + pin_width*0.5, pin_y + pin_width*0.5, z ); 		    fprintf( pPin,
// "%.3f, %.3f, %.3f\n",   pin_x + pin_width*0.5, pin_y - pin_width*0.5, z );
// 		    fprintf( pPin, "%.3f, %.3f, %.3f\n\n", pin_x - pin_width*0.5,
// pin_y - pin_width*0.5, z );
// 		}
// 	    }
// 	}
//     }
...
//     if (withZoom)
//     {
// 	fprintf( pPlt, "plot[:][:] '%s' w l 3, '%s' w l 4, '%s' w l 1, '%s' w l
// 7, '%s' w l 5, '%s' w l 2\n\n", 		fixed_filename.c_str(),
// module_filename.c_str(), move_filename.c_str(), 		net_filename.c_str(),
// dummy_filename.c_str(), pin_filename.c_str() );
//     }
...
// 	int type = 0;
// 	if( gArg.CheckExist( "cong" ) && (int)m_modules.size()< 50000 ) //
// filled color 	    type = 1; 	fprintf( pPlt, "plot[:][:] '-' w l 3, '-' w %s 4, '-'
// w l 1, '-' w l 7, '-' w l 5, '-' w l 2\n\n", 		drawType[type].c_str() );
...
// // nicky 2007-01-22
// void CPlaceDB::OutputPngFigure( const char* prefix, bool withCellMove, bool
// showMsg, bool withOrient , bool withPin )
// {
...
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.8, y+h );
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.8 - w*0.1, y+h -
// h*0.1 ); 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.8, y+h ); 			    fprintf( pOrient,
// "%.3f %.3f\n\n", x+w*0.8 + w*0.1, y+h - h*0.1 ); 			    break; 			case 1:
// //W 			    fprintf( pOrient, "5\n"); 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.5,
// y+h*0.8 ); 			    fprintf( pOrient, "%.3f %.3f\n", x, y + h*0.8 ); 			    fprintf( pOrient,
// "%.3f %.3f\n", x+w*0.1, y+h*0.9 ); 			    fprintf( pOrient, "%.3f %.3f\n", x, y +
// h*0.8 ); 			    fprintf( pOrient, "%.3f %.3f\n\n", x + w*0.1, y + h*0.7 ); 			    break;
// 			case 2:		//S
// 			    fprintf( pOrient, "5\n");
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.2, y+h*0.5 );
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.2, y );
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.2-w*0.1, y+h*0.1
// ); 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.2, y ); 			    fprintf( pOrient, "%.3f
// %.3f\n\n", x+w*0.2 + w*0.1, y+h*0.1 ); 			    break; 			case 3:		//E
// 			    fprintf( pOrient, "5\n");
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.5, y+h*0.2 );
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w, y+h*0.2 );
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w - w * 0.1, y+h*0.2 +
...
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.2, y + h );
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.2-w*0.1, y + h*0.9
// ); 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.2, y + h ); 			    fprintf( pOrient, "%.3f
// %.3f\n", x+w*0.2 + w*0.1, y + h*0.9 ); 			    fprintf( pOrient, "%.3f %.3f\n\n",
// x+w*0.2-w*0.1, y + h*0.9 ); 			    break; 			case 5:		//FW 			    fprintf( pOrient,
// "6\n"); 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.5, y+h*0.8 ); 			    fprintf(
// pOrient, "%.3f %.3f\n", x+w, y + h *0.8 ); 			    fprintf( pOrient, "%.3f %.3f\n",
// x+w-w*0.1, y + h *0.9 ); 			    fprintf( pOrient, "%.3f %.3f\n", x+w, y + h *0.8 );
// 			    fprintf( pOrient, "%.3f %.3f\n", x+w -w*0.1, y + h *0.7
// ); 			    fprintf( pOrient, "%.3f %.3f\n\n", x+w-w*0.1, y + h *0.9 ); 			    break; 			case 6:
// //FS 			    fprintf( pOrient, "6\n"); 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.8,
// y+h*0.5 ); 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.8, y  ); 			    fprintf( pOrient,
// "%.3f %.3f\n", x+w*0.7, y + h * 0.1 ); 			    fprintf( pOrient, "%.3f %.3f\n",
// x+w*0.8, y ); 			    fprintf( pOrient, "%.3f %.3f\n", x+w*0.9, y + h * 0.1);
// 			    fprintf( pOrient, "%.3f %.3f\n\n", x+w*0.7, y + h * 0.1
// ); 			    break; 			case 7:		//FE 			    fprintf( pOrient, "6\n"); 			    fprintf( pOrient,
// "%.3f %.3f\n", x+w*0.5, y+h*0.2 ); 			    fprintf( pOrient, "%.3f %.3f\n", x, y +
// h*0.2  ); 			    fprintf( pOrient, "%.3f %.3f\n", x + w*0.1, y + h*0.1  ); 			    fprintf(
// pOrient, "%.3f %.3f\n", x, y + h*0.2  ); 			    fprintf( pOrient, "%.3f %.3f\n", x +
// w*0.1, y + h*0.3  ); 			    fprintf( pOrient, "%.3f %.3f\n\n", x + w*0.1, y + h*0.1
// ); 			    break;
...
// 	    fprintf( pImage, "%d %d\n", macro_color, macro_mode);  // first set
// color; second set mode
...
// ); 			fprintf( pOrient, "%.3f %.3f\n", x+w*0.8, y+h ); 			fprintf( pOrient, "%.3f
// %.3f\n\n", x+w*0.8 + w*0.1, y+h - h*0.1 ); 			break; 		    case 1:		//W
// 			fprintf( pOrient, "5\n");
// 			fprintf( pOrient, "%.3f %.3f\n", x+w*0.5, y+h*0.8 );
// 			fprintf( pOrient, "%.3f %.3f\n", x, y + h*0.8 );
...
// 			fprintf( pOrient, "%.3f %.3f\n", x, y + h*0.8 );
// 			fprintf( pOrient, "%.3f %.3f\n\n", x + w*0.1, y + h*0.7
// ); 			break; 		    case 2:		//S 			fprintf( pOrient, "5\n"); 			fprintf( pOrient,
// "%.3f %.3f\n", x+w*0.2, y+h*0.5 ); 			fprintf( pOrient, "%.3f %.3f\n", x+w*0.2,
// y ); 			fprintf( pOrient, "%.3f %.3f\n", x+w*0.2-w*0.1, y+h*0.1 ); 			fprintf(
// pOrient, "%.3f %.3f\n", x+w*0.2, y ); 			fprintf( pOrient, "%.3f %.3f\n\n",
// x+w*0.2 + w*0.1, y+h*0.1 ); 			break; 		    case 3:		//E 			fprintf( pOrient,
// "5\n"); 			fprintf( pOrient, "%.3f %.3f\n", x+w*0.5, y+h*0.2 ); 			fprintf(
// pOrient, "%.3f %.3f\n", x+w, y+h*0.2 ); 			fprintf( pOrient, "%.3f %.3f\n", x+w
// - w * 0.1, y+h*0.2 + h * 0.1  ); 			fprintf( pOrient, "%.3f %.3f\n", x+w,
// y+h*0.2 ); 			fprintf( pOrient, "%.3f %.3f\n\n", x+w - w * 0.1, y+h*0.2 - h *
// 0.1 );
...
// pin_y + pin_width*0.5 ); 		    fprintf( pPin, "%.3f %.3f\n",   pin_x +
// pin_width*0.5, pin_y - pin_width*0.5 ); 		    fprintf( pPin, "%.3f %.3f\n",   pin_x
// - pin_width*0.5, pin_y - pin_width*0.5 );
// 		}
// 	    }
// 	}
//     }
...
void CPlaceDB::RemoveFixedBlockSite() {
  ...
  for (vector<Module>::iterator iteModule = m_modules.begin();
       iteModule < m_modules.end(); iteModule++) {
    ...
    // kaie
    if ((int)iteModule->m_z != m_layer)
      continue;
    // @kaie

    if (iteModule->m_isFixed &&    // Find a fixed module
        !iteModule->m_isNI &&      // skip terminal_NI
        module_bottom < row_top && // Confirm that the module is overlapped with
                                   // the rows in y coordinates
        module_top > row_bottom) {
      vector<CSiteRow>::iterator iteBeginRow, iteEndRow;

      // find the begin row
      for (iteBeginRow = m_sites.begin(); iteBeginRow < m_sites.end();
           iteBeginRow++) {
        if (iteBeginRow->m_bottom + iteBeginRow->m_height > module_bottom)
          break;
      }

      for (iteEndRow = iteBeginRow; iteEndRow < m_sites.end(); iteEndRow++) {
      ...
      for (vector<CSiteRow>::iterator iteRow = iteBeginRow; iteRow <= iteEndRow;
           iteRow++) {
        double interval[2];
        for (int i = 0; i < (signed)iteRow->m_interval.size(); i++) {
          ...
          {
            if (interval[0] >= module_right ||
                interval[1] <= module_left) // screen unnecessary checks
              continue;

            if (interval[0] >= module_left && interval[1] <= module_right) {
              //    ---
              // MMMMMMMMM
              iteRow->m_interval.erase(
                  vector<double>::iterator(&(iteRow->m_interval[i])));
              iteRow->m_interval.erase(
                  vector<double>::iterator(&(iteRow->m_interval[i - 1])));
              i = i - 2;
            } else if (interval[1] > module_right &&
                       interval[0] >= module_left) {
              // ------      -----
              // MMM      MMMMM
              iteRow->m_interval[i - 1] = module_right;
            } else if (interval[0] < module_left &&
                       interval[1] <= module_right) {
              // ---------       -----
              //     MMMMM          MMMMM
...
int CPlaceDB::Align() {
  ...
  for (unsigned int i = 0; i < m_modules.size(); i++) {
    ...
    if (!curModule.m_isFixed) {
      ...
      n_slot =
          floor(((curModule.m_x + (site_step / 2.0)) / site_step)); // by donnie
      aligned_locx = n_slot * site_step;
...
bool CPlaceDB::ModuleInCore(const int &i) {
  ...
  if (x >= m_coreRgn.left && x <= m_coreRgn.right && y >= m_coreRgn.bottom &&
      y <= m_coreRgn.top)
    return true;
  else
    return false;
...
void CPlaceDB::ShowDBInfo() {
  ...
  double coreArea =
      (m_coreRgn.right - m_coreRgn.left) * (m_coreRgn.top - m_coreRgn.bottom);
  ...
  double fixedAreaInCore = GetFixBlockArea(m_coreRgn.left, m_coreRgn.bottom,
                                           m_coreRgn.right, m_coreRgn.top);
  ...
  // printf( "           Site Area: %.0f (%.0f)", totalSiteArea,
  // coreArea-fixedAreaInCore );
  printf("              Cell #: %d (=%dk)\n", nCell, (nCell / 1000));
  printf("            Object #: %d (=%dk) (fixed: %d) (macro: %d)\n", modNum,
         modNum / 1000, nFixed, nMacro);
  if (nMacro < 20) {
    for (int i = 0; i < (int)m_modules.size(); ++i)
      if (m_modules[i].m_height > m_rowHeight)
        printf(" Macro: %s\n", m_modules[i].GetName().c_str());
...
//     mSteinerWL, mSteinerWL/hpwl ); printf( "        X Steiner WL: %.0f
...
void CPlaceDB::AddNet(Net n) { m_nets.push_back(n); }
...
// Memory allocation
void CPlaceDB::ReserveModuleMemory(const int &n) { m_modules.reserve(n); }
void CPlaceDB::ReserveNetMemory(const int &n) { CPlaceDB::m_nets.reserve(n); }
void CPlaceDB::ReservePinMemory(const int &n) { m_pins.reserve(n); }

void CPlaceDB::RemoveDummyFixedBlock() { m_modules.resize(realModuleNumber); }
  ...
  for (int i = 0; i < (int)m_sites.size(); i++) {
    ...
    for (int j = 0; j < (int)m_sites[i].m_interval.size(); j += 2) {
      if (m_sites[i].m_interval[j] > currentX) {
        ...
        AddModule(name, float(m_sites[i].m_interval[j] - currentX),
                  m_sites[i].m_height, true);
        SetModuleLocation((int)m_modules.size() - 1, currentX,
                          m_sites[i].m_bottom);
        counter++;
      }
  ...
  for (vector<CSiteRow>::iterator iteSite = m_sites.begin();
    ...
    for (unsigned int i = 0; i < iteSite->m_interval.size(); i = i + 2) {
      ...
      if (old_width < site_step) {
        iteSite->m_interval.erase(
            static_cast<vector<double>::iterator>(&iteSite->m_interval[i + 1]));
        iteSite->m_interval.erase(
            static_cast<vector<double>::iterator>(&iteSite->m_interval[i]));
        i = i - 2;
        continue;
      }

      double new_left =
          ceil((interval_left - chip_left_bound) / site_step) * site_step +
          chip_left_bound;
      double new_right =
          floor((interval_right - chip_left_bound) / site_step) * site_step +
          chip_left_bound;
      if (fabs(new_left - interval_left - site_step) < 1.0e-10)
        new_left = interval_left;
      if (fabs(interval_right - new_right - site_step) < 1.0e-10)
        new_right = interval_right;
      // printf("%lf, %lf\n", (interval_right-chip_left_bound)/site_step,
      // floor((interval_right-chip_left_bound)/site_step));

      double new_width = new_right - new_left;
      if (new_width > 0) {
        // printf("(%lf) %lf-%lf => %lf-%lf\n", chip_left_bound,
        // iteSite->m_interval[i], iteSite->m_interval[i+1], new_left,
        // new_right);
        ...
        if (new_width < 0) {
          fprintf(stderr, "Warning: FixFreeSiteBySiteStep() computes illegal "
                          "site interval\n");
          fprintf(stderr,
                  " site bottom: %.2f orig left: %.2f orig right: %.2f new "
                  "left: %.2f new right: %.2f\n",
                  iteSite->m_bottom, interval_left, interval_right, new_left,
                  new_right);
...
double CPlaceDB::GetXNetLength(vector<int> &pinsId) {
  ...
  double XHPWL = (sqrt(2) - 1) * (x2 - x1 + y2 - y1) -
...
double CPlaceDB::GetNetLpNorm(int netId) {
  ...
  // x_max ,maxX ,( x_max -maxX ) / maxX  ,x_min , minX , (x_min-minX) / minX);

  // 	printf(" %d: Y MAX  %f %f (%f) MIN %f %f (%f)\n", m_nets[netId].size(),
  // y_max ,maxY ,( y_max -maxY ) / maxY  ,y_min , minY , (y_min-minY) / minY);
  hpwl = maxX - minX + maxY - minY;
  _lpnorm = (x_max - x_min + y_max - y_min) / scale;

  // 	printf(" %d: %f %f (%f) \n",netId, _lpnorm , hpwl , (_lpnorm - hpwl ) /
  // hpwl);
...
// 	{
// 	    if( m_pLEF->m_modules[ m_modules[i].m_lefCellId ].m_name.substr( 0,
// 3 ) == "DFF" )
// 	    {
...
int CPlaceDB::GetNetIdByBlockAndPin(const int &blockId, const int &pinId) {
  ...
  // floating? 	    m_modules[blockId].m_name.c_str(), blockId,
...
// 	if( i%1000 == 0 )
// 	    printf( "%d of %d ... (%.0f)(%.0f)\n", i, (int)m_nets.size(),
// totalXSteinerWL, totalMSTwire );

// 	set< pair<double, double> > setTerminals;
// 	for( unsigned int j=0; j<m_nets[i].size(); j++ )
// 	{
// 	    int pinId = m_nets[i][j];
// 	    //terminals.push_back( CPoint( m_pins[pinId].absX,
// m_pins[pinId].absY ) ); 	    setTerminals.insert( make_pair( m_pins[pinId].absX,
// m_pins[pinId].absY ) );
// 	}
...
// 	{
// 	    zeroCount++;
// 	    printf( "(%d) net [%d] mst_seg %d", zeroCount, i,
// mst.m_mst_array.size() );
...
// 	    {
// 		double xDiff = fabs( xsteiner.m_xst_array[j].x -
// xsteiner.m_xst_array[j+1].x ); 		double yDiff = fabs( xsteiner.m_xst_array[j].y
// - xsteiner.m_xst_array[j+1].y ); 		totalXSteinerWL += sqrt( xDiff * xDiff +
// yDiff * yDiff );
// 	    }
// 	}
//     }
...
// double CPlaceDB::GetSteinerWL( const Net& curNet, int steinerType/*=0*/, int
// routingType/*=0*/ )
// {
//     // steinerType 0    FLUTE
...
// ); 	    else 		wire += CPoint::Distance( mst.m_mst_array[j], mst.m_mst_array[j+1] );
// 	}
//     }
//     else
//     {
...
// 	{
// 	    if( routingType == 1 )
// 		wire += CPoint::XDistance( pgFlute->m_steiner_array[j],
// pgFlute->m_steiner_array[j+1] ); 	    else 		wire += CPoint::Distance(
// pgFlute->m_steiner_array[j], pgFlute->m_steiner_array[j+1] );
// 	}
//     }
//     return wire;
// }

// double CPlaceDB::GetTotalSteinerWL( const int& steinerType, const int&
// routingType )
// {
//     double wire = 0;
//     for( unsigned int i=0; i<m_nets.size(); i++ )
...
void CPlaceDB::AdjustStandardCellOrientation() {
  for (unsigned int i = 0; i < m_modules.size(); i++) {
    ...
    if (row_id % 2 == 0)
      SetModuleOrientationCenter(i, OR_FS);
    else
      SetModuleOrientationCenter(i, OR_N);
  }
}
...
void CPlaceDB::AdjustNetConnection() {
  ...
  for (unsigned int i = 0; i < m_modules.size(); i++) {
    for (unsigned int j = 0; j < m_modules[i].m_pinsId.size(); j++) {
      int pinId = m_modules[i].m_pinsId[j];
      int netId = pinNet[pinId];
      if (pinId >= 0 && pinId < (int)m_pins.size() && netId >= 0 &&
          netId < (int)m_nets.size())
  ...
  for (unsigned int i = 0; i < m_nets.size(); i++) {
    for (unsigned int j = 0; j < m_nets[i].size(); j++) {
      int pinId = m_nets[i][j];
      if (visited[pinId] == true)
        printf("pin %d redefined!\n", pinId);
      else
        visited[pinId] = true;
    }
  }
...
void CPlaceDB::ResizeCoreRegion3d(const double wsratio) {
  printf("original core region: (%.2f - %.2f)(%.2f - %.2f)\n", m_coreRgn.left,
         m_coreRgn.right, m_coreRgn.bottom, m_coreRgn.top);
  ...
  int count = 0;
  while (true) {
    if (row_bottom + row_height > m_coreRgn.top)
      break;
  ...
  // for(unsigned int i = 0; i < m_sites3d[0].size(); i++)
  // printf("CORE_ROW_%d: %lf -- %lf\n", i, m_sites3d[0][i].m_interval[0],
  // m_sites3d[0][i].m_interval[1]);
  for (int l = 1; l < m_totalLayer; l++)
...
void CPlaceDB::HMETIS_PartKway(int nvtxs, int nhedges, int *vwgts, int *eptr,
                               int *eind, int *hewgts, int nparts, int ubfactor,
                               int *options, int *part, int *edgecut) {
  /// TODO: write file --> run khmetis.exe --> read file
}
void CPlaceDB::HMETIS_PartRecursive(int nvtxs, int nhedges, int *vwgts,
...
void FM_Partition::PrintPartitions() {
  ...
  printf("|A| = %d\n", partition_A.size());
  printf("A = { ");
  for (theIterator = partition_A.begin(); theIterator != partition_A.end();
       theIterator++)
    printf("c%d ", *(theIterator));
  printf("}\n");
  printf("|B| = %d\n", partition_B.size());
  printf("B = { ");
  for (theIterator = partition_B.begin(); theIterator != partition_B.end();
       theIterator++)
    printf("c%d ", *(theIterator));
  printf("}\n");
}
...
void FM_Partition::PrintGainBucket() {
  ...
  for (int i = 0; i < (int)gain_bucket.size(); i++) {
    ...
    for (Literator = gain_bucket[i].begin(); Literator != gain_bucket[i].end();
         Literator++) {
...
void FM_Partition::init() {
  ...
  while (partition_A_size < (total_area / 2)) {
    ...
    for (unsigned int j = 0; j < cells[i + 1].netId.size();
         j++) // count for nets F(n) and T(n)
  ...
  {
          partition_A.insert(cells[i+1].id);      // insert cell[i+1] to
  partition A cells[i+1].block = BLK_A;               // mark cell[i+1]'s block
  by A for(unsigned int j = 0; j < cells[i+1].netId.size(); j++) // count for
  nets F(n) and T(n) nets[cells[i+1].netId[j]].block_cells_bak[BLK_A]++;
  }
  for(i = partition_num; i < num_cells; i++)
  {
          partition_B.insert(cells[i+1].id);      // insert cell[i+1] to
  partition B cells[i+1].block = BLK_B;               // mark cell[i+1]'s block
  by B for(unsigned int j = 0; j < cells[i+1].netId.size(); j++) // count for
  nets F(n) and T(n) nets[cells[i+1].netId[j]].block_cells_bak[BLK_B]++;
  }*/
  ...
  /*for(int net_id = 0; net_id < num_nets; net_id++)      // check block_cells's
    consistency
    {
    assert(nets[net_id].block_cells_bak[BLK_A]+nets[net_id].block_cells_bak[BLK_B]
    == nets[net_id].cellId.size());
    }*/
  ...
  for (int i = 0; i < num_nets; i++) {
    // net has cells in both sides
    if (nets[i].block_cells_bak[BLK_A] > 0 &&
        nets[i].block_cells_bak[BLK_B] > 0) {
      cut.push_back(i);
      cut_size++;
    }
  }
...
void FM_Partition::InitCellGains() {
  ...
  for (int cell_id = 1; cell_id <= num_cells; cell_id++) {
    ...
    for (unsigned int j = 0; j < cells[cell_id].netId.size(); j++) {
      ...
      if (nets[net_id].block_cells[from_block] == 1)
        cells[cell_id].gain++;
    ...
    // to_block, cells[cell_id].gain); assert(p_max >= cells[cell_id].gain);
...
void FM_Partition::UpdateCellGains(int base_cell) {
  ...
  for (unsigned int i = 0; i < cells[base_cell].netId.size(); i++) {
    ...
    // printf("%s, F = %d, T = %d\n", nets[net_id].name.c_str(),
    // nets[net_id].block_cells[from_block], nets[net_id].block_cells[to_block]);
    ...
    if (nets[net_id].block_cells[to_block] == 0) {
      for (unsigned int j = 0; j < nets[net_id].cellId.size(); j++) {
        ...
        if (cells[cell_id].isFree) {
          UpdateGainBucket(cell_id, cells[cell_id].gain,
                           cells[cell_id].gain + 1);
          cells[cell_id].gain++;
        }
      }
    }
...
int FM_Partition::Par() {
  ...
  while (1) {
    ...
    for (unsigned int i = max_gain_ptr; i < gain_bucket.size(); i++) {
      int j;
      if (gain_bucket[i].size() == 0)
        continue;
      for (Literator = gain_bucket[i].begin(), j = 0;
           Literator != gain_bucket[i].end(); Literator++, j++) {
    ...
    solutions[num_solutions] =
        FMSolution(gain, base_cell, partition_A_size, partition_B_size);

    double balance = abs(partition_A_size - partition_B_size);
    // int balance = abs((int)partition_A_size-(int)partition_B_size);
    if (solutions[num_solutions].gain > max_gain ||
        (solutions[num_solutions].gain == max_gain && balance < max_balance)) {
